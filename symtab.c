...
/*int insert_class(char *name, unsigned kind, unsigned access_specifier){
	int index = get_next_empty_element();
  symbol_table[index].name = name;
  symbol_table[index].kind = kind;
	symbol_table[index].type = access_specifier;
  return index;
}*/
...
/*unsigned get_access_specifier(int index) {
	if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].kind;
  return NO_KIND;
}
...
void print_symtab(void) {
  static const char *symbol_kinds[] = { 
    "NONE", "REG", "LIT", "FUN", "VAR", "PAR", "CLS", "CLS_ATR", "CONSTR" };
  int i,j;
  printf("\n\nSYMBOL TABLE\n");
